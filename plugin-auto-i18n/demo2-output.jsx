...
import { toFixedColumns, doReceiptCheck, testDateRange, testDateRange2, cancelSeeDoctor, cancelReadCard, doReceiptCheckType5, hardwareCardCheckreading, doHardwareDispenserCheckstate, handlePatientNameDesensitization, noPassByNameExart } from '../lib/util';
...
import Loading from '../component/base/Loading'; // import '../component/less/4_1_0-index-page-zjp4-2-0.css'
...
class IndexPage extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      unPayRecords: false,
      isGrayMask: false,
      //默认不置灰 优先处理缴费单后处理预约记录
      queryUserInfoLoading: false,
      isCompleteUserInfo: true,
      //默认显示强制待办 需要补充信息时暂时隐藏
      secondFuncList: [],
      //次级功能
      showSecondList: false,
      //显示更多功能栏
      useType4_1_0: props.config.useType4_1_0 || false,
      //版本控制
      printExeStatus: false,
      //激光打印机状态
  ...
  setSecondFuncList = (config = {}) => {
    ...
    secondFunc = functionArray.map((item, key) => item).filter(item => item.location == undefined) || {}; //过滤没有location的func 返回func
  ...
  setAnimation = (queryParams, animationList) => {
    ...
    if (key in indexLoopVersion2Config[animationList]) {
      let animation = indexLoopVersion2Config[animationList][key].animation || '';
      let animationTitle = indexLoopVersion2Config[animationList][key].animationTitle || '';
      this.setState({
        animation,
        animationTitle
      });
  ...
  ForcePayFor1040 = () => {
    ...
    return <div className='indexpage-force-pay'>
        <div className='indexpage-force-pay-content'>
          <div className='indexpage-force-pay-title'>{_intl.t('待办理项')}</div>
          <div className='indexpage-force-pay-button'>
            {unPayRecords && <button onClick={() => this.recordsClick()} className={`indexpage-force-pay-records ${isGrayMask ? 'indexpage-force-pay-records-gray' : ''}`}>{_intl.t('预约记录')}<div className='force-pay-info-text-confirm'>{_intl.t('点击确认')}</div>
              </button>}
            {unPayPayment && <button onClick={() => this.paymentClick()} className={`indexpage-force-pay-payment`}>{_intl.t('医疗缴费')}<div className='force-pay-info-text-confirm'>{_intl.t('点击确认')}</div>
              </button>}
          </div>
          <Button className='indexpage-force-pay-exitCard' onTouchEnd={this.handleCardCheckout.bind(this)}>
            <pre>{_intl.t('退 卡')}</pre>
          </Button>
        </div>
      </div>;
  ...
  inital = () => {
    ...
    actions.registration.setLoading({
      unLockSource: false
    });
    ...
    actions.store.setAptSelectedStatus(''); //青岛妇儿 缓存预约信息 end
    ...
    actions.store.setMedicalCardInfo({}); // 清空住院病人查询的参数缓存

    actions.inHos.setQueryUserInfoParams({}); // 首页清除代付卡信息

    actions.deviceExtra.setCardReadResult({}); // 清除预处理信息

    actions.pay.setPreProcess({});
    actions.pay.setQueryOrderStatus({}); // 清空缓存的充值类型  store.type

    actions.store.setSelectType(''); // 清空缴费的 extend 信息
    ...
    actions.docCard.doSetSetupType({
      setupType: '1'
    });
    actions.docCard.doSetSetupCardType({
      setupCardType: ''
    });
    ...
    actions.store.setReceiptInfo({
      printTitle: _intl.t('正在打印凭条...')
    }); // 进入页面时 清除扫码暂存的读卡数据

    actions.scan.clearCardInfo(); // 重置当前病人信息查询为 门诊病人

    actions.store.doSetCurrentQueryType({
      currentQueryType: '1'
    }); // 重置查询缴费单接口是否结算状态

    actions.search.setSearchPayType(''); // 重置科室查询接口类型
  ...
  handleUserClear = () => {
    const {
      indexLoopVersion2 = false
    } = this.props.config; // 自动卡识别不主动清除病人信息 / 住院病人信息
    ...
    if (!!removeUserInfoIndex || !!cardType && this.elecCardList.indexOf(cardType.toString()) > -1) {
      // 903医院首页清空用户信息
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if ('indexLoopVersion2' in currentConfig && 'indexLoopVersion2' in nextConfig && currentConfig.indexLoopVersion2 !== nextConfig.indexLoopVersion2 && nextConfig.indexLoopVersion2 === false) {
      // 是否轮询配置变更 - 去掉轮询定时器
      this.timer && clearInterval(this.timer);
      this.timer = null;
    }
    ...
    } = nextProps.config; // _timeStamp 表示 当前配置文件版本的时间戳
    ...
    try {
      ...
      console.error('Error in indexPage.zjp_v4_0_0.js componentWillReceiveProps');
    }
  }
  ...
  async componentDidMount() {
    ...
    this.initalMonitor(); //激光打印流程优化

    this.printExeOptimization(); // 2021.02.24 硬件检测 start
    ...
    if (useType4_1_0) {
      ...
      this.mainStyleList = this.renderMainStyle(this.props.config); // 2020.10.22 修改 从 constructor 中改为 放到  componentDidMount

      this.setSecondFuncList(this.props.config);
    }

    const {
      cardInfo: cardInfoExtra = {}
    } = this.props.deviceExtra;
    const {
      cardNo: cardNoExtra = '',
    ...
    const {
      isCompleteUserInfo = true
    } = this.state; // 如果为 扫码的  诊疗卡/社保卡/身份证 进入功能页时，清空信息
    ...
    if (isQrCode || !cardType || cardType == 1 || cardType == 16 || ['54', '55'].includes(String(cardType))) {
    ...
    this.setState({
      todoDrawerJudge: true
    }); // 确保待办数据接口调用顺序

    if (['345', '261', '346', '344', '341'].includes(String(window.hospitalId))) {
      //检测到卡信息 弹框提示
      const {
        cardInfo = {}
      } = this.props.device;

      if (!autoExitCard && Object.keys(cardInfo).length > 0 && cardInfo.isQrCode != '1' && ['17', '18', '24', '54', '20', '1'].includes(String(cardInfo.cardType)) === false // 不为电子卡时
      ) {
        Notice.confirm({
          content: _intl.t('检测到您的卡信息，是否立即退卡'),
          duration: 10000,
          cancelBtnText: _intl.t('否'),
          successBtnText: _intl.t('立即退卡'),
          onClose: () => {
            Notice.hide();
          },
          onSuccess: async () => {
    ...
    if (['1040', '1045'].includes(String(window.hospitalId)) && useForcePayFor580 && isCompleteUserInfo && !!info.patientId) {
      ...
      console.log(_intl.t('返回首页退卡卡类型：'), cardType);

      if (cardType == 2 && cardType2WithExit || cardType == 10 && cardType10WithExit || cardType == 11 && cardType11WithExit) {
        actions.user.getCheckoutCard({
          cardType
        }).then(res => res).catch(res => res);
      }
    ...
    actions.scan.clearCardInfo(); // 维护模式 添加
    ...
    let cardReader = this.props.config.cardReader[_intl.t('门诊')] || {};
    let method = Object.keys(cardReader).find(key => {
      return cardReader[key].indexLogin;
    }); //多个卡类型

    let cards = cardReader[method]['cardType'].toString().split(','); // 测试不用
    ...
    }, doWhileTime); // window.getPatientInfoByCard = this.getPatientInfoByCard;
    // 当页面页面路径传入 CardNo CardType的时候  可以是用 这两个参数 作为mock 的卡号
  }

  printExeOptimization = async () => {
    const {
      printExeTestingOptimization = false
    } = this.props.config;
  ...
  handleIndexLoopVersion2 = () => {
    ...
    if (useLocalServiceVersion2) {
      ...
      let reqsArr = [''].map((item, index) => {
        ...
        let params = {}; // let queryList = String(physicalCardList).split(',') || []
        // if(window.hospitalId == '1109' && queryList.includes(String(10))){
        //   params.transType = 'ReadCard'
        // }

        return actions.device.getCardReadResult({ ...params,
    ...
    if (useLocalServiceVersion2 && ['1045'].includes(String(window.hospitalId))) {
      //调小远助手时 合并查询
  ...
  getUserInfoByPromiseAllVersion2 = reqsArr => {
    ...
    const {
      hospitalId = ''
    } = corpInfo; // 读卡接口返回后 判断有无读取到信息

    Promise.all(reqsArr).then(async res => {
      ...
      if (successRes.length > 0) {
        ...
        if (!cardNo && (cardType == 2 || indexUseType2And10 && cardType == 10)) {
          // 1.读卡成功，但是 卡为空卡， 需要提示退卡
          if (['1040', '1045'].includes(String(window.hospitalId)) && cardType == 10) {//湖州中心医院 第三代社保卡his读卡 只有cardType 没有cardNo
          } else {
            Notice.alert({
              content: _intl.t('未能读取到卡号，请按画面提示重新插卡，检查卡是否插反'),
              duration: 5000,
              onClose: () => {
        ...
        actions.polling.setGettingUserInfo(true); // const { isLogin = false } = await getUserInfoCommon({queryParams: data})
        ...
        const {
          ...
          outpatientInfoCode = '',
          inhosInfoMsg = ''
        } = await window.userInfoUtil.subscribe({
          outpatientUserInfoStart: ({
            queryParams = {}
          }) => {
            // 门诊病人信息开始
            // console.log('outpatientUserInfoStart')
            this.setAnimation(queryParams, 'outpatientUserInfoAnimationList');
          },
          outpatientUserInfoEnd: ({
            queryParams = {}
          }) => {
            // 门诊病人信息结束
        ...
        const {
          completeAddressAndPhone = false
        } = this.props.config;
        const {
          info = {}
        } = this.props.user; //重庆四院支持 单张和多张缴费两种模式 根据病人卡类型来判断是否单张或者多张
        ...
        if (completeAddressAndPhone && !['1030'].includes(String(window.hospitalId))) {
          const {
            info = {}
          } = this.props.user;
          const {
          ...
          if (!!infoKey && Object.keys(infoKey).length > 0) {
            const {
              completeByDocCardOrHandInPut = false
            } = this.props.config;
            const {
              phone = '',
              idNo = '',
            ...
            if (completeByDocCardOrHandInPut && !address) {
              ...
              Notice.alert({
                content: _intl.t('您的信息填写不完整，请前往补全！'),
                duration: 5000,
                onClose: () => {
            ...
            if (['150'].includes(String(window.hospitalId)) && (!phone || !idNo)) {
              if (!idNo || !phone) {
                const {
                  selectedFunc = {}
                } = this.props.store;
                ...
                if (func.label && func.label == _intl.t('自助充值')) {
                  Notice.alert({
                    title: _intl.t('提示'),
                    content: <Fragment>
                          <div style={{
                        textAlign: 'left',
                        padding: '10px 25px'
                      }}>{_intl.t('1.您的档案缺少信息，为方便您在本院就医，我们推荐您补充完整信息；')}</div>
                          <div style={{
                        textAlign: 'left',
                        padding: '10px 25px'
                      }}>{_intl.t('2.若您没带身份证可手动或者前往窗口补充，然后继续操作。')}</div>
                        </Fragment>,
                    duration: 60000,
                    showMask: true,
                    cancelBtnText: _intl.t('补充信息'),
                    onClose: () => {
                  ...
                  Notice.confirm({
                    ...
                    content: <Fragment>
                        ...
                        textAlign: 'left',
                        padding: '10px 25px'
                      }}>{_intl.t('您的档案缺少信息，为方便您在本院就医，我们推荐您补充完整信息；')}</div>
                          <div style={{
                        textAlign: 'left',
                        padding: '10px 25px'
                      }}>{_intl.t('2.若您没带身份证可手动或者前往窗口补充，然后继续操作。')}</div>
                    ...
                    duration: 60000,
                    showMask: true,
                    cancelBtnText: _intl.t('取消'),
                    successBtnText: _intl.t('补充信息'),
                    onClose: () => {
                      Notice.hide(); // actions.routing.replace("/indexPage")
                    },
                    onSuccess: () => {
                      actions.routing.push('/loginSelector/completeUserInfo');
                    }
        ...
        if (window.hospitalId == '1002' && Object.keys(info).length > 0 && !info.phone) {
          Notice.alert({
            content: _intl.t('您的信息填写不完整，请前往补全！'),
            duration: 5000,
            onClose: () => {
        ...
        if (isLogin === false) {
          ...
          if (useLocalServiceVersion2) {
            // 2020.08.10 版本 本地服务
            let cardType2Bool = ['-2'].includes(String(outpatientInfoCode)) && cardType == '2';
            let cardType10Bool = outpatientInfoCode == '-2' && cardType == '10';

            if ((cardType2Bool || cardType10Bool) && type != 'ZJP') {
              // 兼容插卡获取不到信息直接退卡
              if (['345', '261', '344', '341', '346'].includes(String(window.hospitalId))) {
                return this.handleCardCheckout(outpatientInfoMsg || inhosInfoMsg || _intl.t('未查询到患者信息，已退卡，检查卡是否正确'));
              }

              return this.handleCardCheckout(_intl.t('未查询到患者信息，已退卡，检查卡是否正确'));
            }
          }

          if ((outpatientInfoCode == '-3' || outpatientInfoCode == '3105') && autoDocCardInCardTypeReader) {
          ...
          Notice.alert({
            content: outpatientInfoMsg || inhosInfoMsg || _intl.t('未查询到患者信息'),
            onClose: () => {
              setTimeout(() => {
                actions.polling.setGettingUserInfo(false);
              }, 2000);
        ...
        if (isLogin) {
          ...
          if (['1040', '1045'].includes(String(window.hospitalId)) && useForcePayFor580 && isCompleteUserInfo) {
            actions.store.setGlobalLoading(true);
            const res1 = await actions.jhRegRecord.getJhRecordList({
              medDate: moment().format('YYYY-MM-DD')
            });
          ...
          if (mobileSupplement && !phone) {
            Notice.confirm({
              title: _intl.t('提示'),
              content: _intl.t('未检测到手机号 需要前往补全'),
              duration: 15000,
              showMask: true,
              cancelBtnText: _intl.t('跳过'),
              successBtnText: _intl.t('前往补全'),
              onClose: async () => {
          ...
          if (window.location.href.indexOf('/indexPage') > -1 === false) {// 查询到病人信息  并且 当前页面不为主页时
        ...
        console.log(_intl.t('轮询未读取到任何卡')); // 继续轮询
        // setTimeout(() => {

        actions.polling.setDoWhile(true); // }, 1000)
      }
    }).catch(err => {
      // 继续轮询
      // setTimeout(() => {
      ...
      if (successRes.length > 0) {
        ...
        if (!!cardNo) {
          ...
          if (!!resInfo.patientId) {
            ...
            // 1.读卡成功，但是 卡为空卡， 需要提示退卡
            Notice.alert({
              content: resInfo.msg || _intl.t('未找到您的就诊信息'),
              duration: 5000,
              onClose: () => {
                this.handleCardCheckout(); // setTimeout(() => {
                //   actions.polling.setDoWhile(true);
                // }, 3000);
              }
            });
        ...
        } else if (!cardNo && (cardType == 2 || indexUseType2And10 && cardType == 10)) {
          // 1.读卡成功，但是 卡为空卡， 需要提示退卡
          Notice.alert({
            content: _intl.t('未能读取到卡号，请按画面提示重新插卡，检查卡是否插反'),
            duration: 5000,
  ...
  getPatientInfoByCard = async cardInfo => {
    ...
    const res = await actions.user.queryPatientInfo({ ...cardInfo,
      patientName: cardInfo.name || ''
    }).then(res => {
      // let patientInfo = {
      //   cardNo: res.seqno,
  ...
  publicfunction(func) {
    ...
    if ('timeLimit' in func && func.timeLimit == true) {
      const {
        morningTime = '',
        afternoonTime
      } = this.props.config; // * 判断时间限制

      const {
        success = false,
        msg = ''
      } = testDateRange(morningTime, afternoonTime);
      ...
      if (!success) {
        ...
        Notice.alert({
          content: <p>
              <span>{_intl.t('当前不在挂号时间')}</span>
              <br />
              <span>{_intl.t('可挂号时间段为：')}{morningTime}，{afternoonTime}
              </span>
            </p>
        });
        return;
    ...
    if (func.forceLogin) {
      ...
      actions.routing.push((func.loginPath || '/login') + this.props.location.search);
      return;
    }
    ...
    if (func.needLogin && !this.props.user.isLogin && func.cardType && func.inputHosId) {// console.log('住院号 卡号同时支持！')
      // return
    ...
    if ((func.path == '/docCard/idCard' || func.path == '/loginSelector/CardTypeSelectorForDocCard') && this.props.user.isLogin) {
      Notice.alert({
        content: _intl.t('您已有账户登录，请退卡后重新建档')
      });
      return;
    } // test 路径加参数
  ...
  async onFunctionClick(func) {
    ...
    if (func.label == _intl.t('更多功能') || func.label == _intl.t('更多')) {
      this.setState({
    ...
    const {
      useLocalServiceVersion2 = false,
      // 使用新本地服务
      useHardwareCardCheckreading = true,
      // 使用跳转前读卡检测  2021.01.05 默认值改为 true，防止漏掉
    ...
    if (['1030'].includes(String(window.hospitalId))) {
      if (!!cardNo && func.label != _intl.t('当天挂号') && (func.label == _intl.t('预约挂号') || func.label == _intl.t('预约取号/取消'))) {
    ...
    if (['1109'].includes(String(window.hospitalId))) {
      ...
      if (!result.success && [_intl.t('自助缴费'), _intl.t('当天挂号'), _intl.t('预约挂号'), _intl.t('住院预缴')].includes(String(func.label))) {
        Notice.alert({
          content: _intl.t('内部系统维护中（23:30-00:15），请前往窗口办理支付业务。带来不便，敬请谅解！')
        });
        return;
      }
    }
    ...
    if (['1040', '1045', '1046'].includes(String(window.hospitalId)) && func.path.indexOf('cash') > -1) {
      const {
        info = {}
      } = this.props.user; //仅开通院内账户时支持充值

      if ('cardStatus' in info && info.cardStatus == '0') {
        Notice.alert({
          content: _intl.t('您尚未开通院内账户，无法充值！'),
          duration: 5000,
          onClose: () => {
            actions.routing.replace('/indexPage');
          }
    ...
    if (['345', '261', '344', '346'].includes(String(window.hospitalId))) {
      ...
      if (info.accountType == '2') {
        //临时卡
        if (func.key == _intl.t('自助充值')) {
          Notice.alert({
            content: _intl.t('临时卡不支持充值业务，请先完实名认证!'),
            duration: 5000,
            onClose: () => {
              Notice.hide();
            }
    ...
    if (window.hospitalId == '1109' && func.label == _intl.t('自助缴费') && Object.keys(cardInfo).length > 0) {
      try {
        const {
          extend = ''
        } = cardInfo;
        let pInfo = !!extend && JSON.parse(extend);
        let illInfo = _intl.t('特慢病信息') in pInfo && !!pInfo.特慢病信息 && pInfo.特慢病信息.data || {};

        if (illInfo.length > 0) {
          Notice.alert({
            content: _intl.t('特慢病患者请前往人工窗口缴费'),
            duration: 5000,
            onClose: () => {
              actions.routing.replace('/indexPage');
            }
    ...
    if (useLocalServiceVersion2 && useHardwareCardCheckreading && indexLoopVersion2 && !!cardNo === false && doWhile === false) {
      // 当前无卡，并且未在轮询时
      // 此处进行判断
      // 如正在轮询中，需要进行卡状态查询
    ...
    const {
      showRegistrationRegType4 = false
    } = this.props.config;

    if (func.label == _intl.t('满意度调查') && ['345', '261', '346', '344', '341'].includes(String(window.hospitalId))) {
      if (window.require && window.require('child_process')) {
        Notice.alert({
          content: _intl.t('调查问卷正在打开，请稍候..')
        });

        const exec = window.require('child_process').exec; // 程序调用路径


        const cmd = `cmd.exe /C start D:\\A王毅\\forewell_nhcrm_pn\\nhcrm_browser.exe`;
        console.log(`${_intl.t('[CallExe]调用满意度调查程序: ')}${cmd}`);
        exec(`${cmd}`);
      } else {
        console.log(_intl.t('[CallExe.js] 请在自助机上测试'));
      }
    ...
    if (func.key === _intl.t('实名认证') && ['261', '346', '344'].includes(String(window.hospitalId))) {
      if (this.props.user.isLogin && info.renamType != -1) {
        return Notice.alert({
          content: _intl.t('当前账户已实名')
        });
      }

      actions.step.setModule(_intl.t('实名认证')); // 定义当前模块类型
    }

    if (func.label === _intl.t('电子健康卡') && 'tips' in func && func.tips) {
      this.setState({
        showTips: true,
        setFunc: func,
        click: true
    ...
    if ('timeLimit2' in func && func.timeLimit2 == true && !showRegistrationRegType4) {
      const {
        allDayTime = ''
      } = this.props.config; // * 判断时间限制
      ...
      if (!success) {
        ...
        Notice.alert({
          content: ['162'].includes(String(window.hospitalId)) ? <p>
              <span>{_intl.t('挂号时间为当日')}{allDayTime}</span>
              <br />
              <span>{_intl.t('请前往二号楼一楼急诊自助机挂急诊号')}</span>
            </p> : <p>
              <span>{_intl.t('当前不在挂号时间')}</span>
              <br />
              <span>{_intl.t('可挂号时间段为：')}{allDayTime}</span>
            </p>,
          duration: 1000 * 15
        });
        return;
    ...
    if ('timeLimitForInhosSelf' in func && func.timeLimitForInhosSelf == true && 'allDayTime' in func && !!func.allDayTime) {
      const {
        allDayTime = ''
      } = func || {}; // * 判断时间限制
      ...
      if (!success) {
        Notice.alert({
          content: <p>
              <span>{_intl.t('自助入院服务时间为：')}{allDayTime}</span>
              <br />
              <span>{_intl.t('其他时间请您到住出院窗口或急诊财务窗口(夜间)咨询办理手续！')}</span>
            </p>,
          duration: 1000 * 15
        });
        return;
    ...
    if ('timeLimitForThisItem' in func && func.timeLimitForThisItem == true && 'allDayTime' in func && !!func.allDayTime) {
      const {
        allDayTime = '',
        timeLimitNoticeForThisItem = ''
      } = func || {}; // * 判断时间限制
      ...
      if (!success) {
        Notice.alert({
          content: <div dangerouslySetInnerHTML={{
            __html: timeLimitNoticeForThisItem
          }} />,
          duration: 1000 * 15
        });
        return;
    ...
    const {
      receiptCheck = false
    } = this.props.config; // 2021.03.25 新增 如无需凭条检测则
    ...
    if ((['/docCard/idCard', '/loginSelector/CardTypeSelectorForDocCard'].includes(func.path) || !!func.noOutpatientUserInfo) && this.props.user.isLogin) {
      Notice.alert({
        content: _intl.t('当前已有账户登录，可直接进行操作')
      });
      return;
    } // 发卡机状态检测


    if ('doHardwareDispenserCheckstate' in func && !!func.doHardwareDispenserCheckstate) {
    ...
    if ('key' in func && func.key == _intl.t('当天挂号') && ['1040'].includes(String(window.hospitalId))) {
      Notice.alert({
        content: _intl.t('如有发热（体温大于37.3℃）请到发热门诊就诊；如有急性腹泻（>3次/天）请到肠道门诊就诊'),
        duration: 5000
      });
    } //衢州中医院 挂号提示


    if ('key' in func && func.key == _intl.t('当天挂号') && ['170'].includes(String(window.hospitalId))) {
      Notice.alert({
        content: _intl.t('胸痛、急性脑卒中患者请到急诊科挂号就诊'),
        duration: 5000
      });
    }

    if ('key' in func && func.key == _intl.t('当天挂号') && ['1045'].includes(String(window.hospitalId))) {
      Notice.alert({
        content: _intl.t('本机构不收治发热病人，如有发热（体温大于37.3℃），请到定点发热医院就诊'),
        duration: 5000
      });
    } // //青岛妇儿医院 挂号提示
    ...
    if (func.path == '/docCard/idCard' || func.path == '/loginSelector/CardTypeSelectorForDocCard') {
  ...
  handleCardCheckout = async noticeText => {
    ...
    if (this.props.device.carReadResult && this.props.device.carReadResult.success) {
      ...
      await actions.user.getCheckoutCard({
        ...
        if (res.success) {
          ...
          const {
            exitBtnText = _intl.t('退卡')
          } = this.props.config;
          Notice.alert({
            icon: 'success',
            content: noticeText ? noticeText : `${exitBtnText}${_intl.t('成功')}`,
            onClose: () => {
  ...
  handleForceExitCardFor1312 = async () => {
    ...
    const res = await api.getCheckoutCard({
      cardType: '2'
    ...
    if (!!res && res.success) {
      Notice.alert({
        icon: 'success',
        content: _intl.t('操作成功')
      });
    } else {
      Notice.alert({
        content: res.msg || _intl.t('操作失败')
      });
    }
  }; // 泸州中医院 强制退卡
  ...
  handleForceExitCardFor713 = async () => {
    ...
    if (useLocalServiceVersion2) {
      // 2020.08.10 版本 本地服务
      reqsArr = api.getCheckoutCard({
        cardType: String(arr)
      });
    } else {
      reqsArr = arr.map((item, index) => {
        return api.getCheckoutCard({
          cardType: item
        });
      });
    }
    ...
    Promise.all(reqsArr).then(async res => {
      ...
      if (successRes.length > 0) {
        ...
        console.log(_intl.t('强制退卡返回：') + JSON.stringify(res));
        Notice.alert({
          content: _intl.t('操作失败')
        });
      }
    }).catch(err => {
      this.setState({
  ...
  handleForceExitCard = async () => {
    ...
    if (['710', '701', '713', '162', '714', '163'].includes(String(window.hospitalId))) {
      // 沙井
      window.userInfoUtil && window.userInfoUtil.init();
  ...
  renderMainStyle = (config = {}) => {
    ...
    firstFuncList = list.map((item, key) => item.location).filter(item => item !== undefined) || {}; //过滤有location的func 返回坐标

    for (let i = 0; i < firstFuncList.length; i++) {
      if (firstFuncList[i].hasOwnProperty('startOffset') && firstFuncList[i].hasOwnProperty('endOffset')) {} else {
        Notice.alert({
          content: _intl.t('配置location但未配置定位元素！'),
          duration: 5000
        });
        this.setState({
  ...
  renderExitCardBtnSlot = () => {
    ...
    if (carReadResult.success) {
      if (cardType == 2 || indexUseType2And10 && (cardType == 2 || cardType == 10 || cardType == 13) || cardType1WithExit && cardType == 1 || cardType2WithExit && cardType == 2 || cardType10WithExit && cardType == 10 || cardType11WithExit && cardType == 11 || cardType16WithExit && cardType == 16 || cardType17WithExit && cardType == 17 || cardType18WithExit && cardType == 18 || cardType24WithExit && cardType == 24 || cardType54WithExit && cardType == 54 || cardType55WithExit && cardType == 55) {
        const {
          exitBtnText = _intl.t('退卡')
        } = this.props.config;
        return <Fragment>
            {useType4_1_0 ? <Button className='new-indexpage-base410-CardCheckout' onTouchEnd={this.handleCardCheckout.bind(this)}>
                {exitBtnText || _intl.t('退卡')}
              </Button> : <Button className='orange-button big-button return-the-card-button' activeClassName='big-button-active' style={{
            position: 'relative',
            zIndex: '12'
  ...
  renderStyle = (config = {}) => {
    ...
    for (let i = 0; i < firstFuncList.length; i++) {
      ...
      if (useThemeVersion) {
        ...
        // }
        commonStyle.borderRadius = '40px';
        commonStyle.background = 'transparent'; // 原来的
        ...
        commonStyle.height = 79 + 111 * (end[0] - start[0]) + 'px';
        commonStyle.top = '0px';
        commonStyle.left = '0px'; // 添加宽度/高度级别

        commonStyle.cardWidthLevel = end[1] - start[1];
        commonStyle.cardheightLevel = end[1] - start[1]; // 测试用 end
        // commonStyle.top = "9px"
        // commonStyle.left = "4px"

        commonStyle.fontSize = firstFuncList[i].fontSize || 44 + 'px'; // commonStyle.justifyContent = 'flex-start' // 带小字时 // TODO
      ...
      if (useThemeVersion2) {
        // todo
        commonStyle.borderRadius = '12px';
        commonStyle.background = 'transparent'; // 测试用 start
  ...
  renderSecondFunc = () => {
    ...
    return <Fragment>
        ...
        return <div key={index} onClick={this.onFunctionClick.bind(this, pageList)} style={pageListStyle} className={`base410-second-func-item total-length-eq-${temp.length} ${pageList.available === false ? 'unavailable' : ''}`}>
              {!!pageList.iconIndex || !!pageList.iconIndexImgUrl ? <div className='base410-second-func-item-1'>
                  <div>
                    {pageList.background ? <img src={!!pageList.iconIndexImgUrl ? pageList.iconIndexImgUrl : useThemeVersion ? iconIndexMapUseTheme[pageList.iconIndex] : iconIndexMap[pageList.iconIndex]} alt='' /> // 有背景色时
              : <img src={!!pageList.iconIndexImgUrl ? pageList.iconIndexImgUrl : useThemeVersion ? iconIndexSecondMap[pageList.iconIndex] : iconIndexSecondMap[pageList.iconIndex]} alt='' /> // 无背景色色时
              }
                  </div>
                ...
                </div> : <div className='base410-second-func-item-2'>
                  <span>{pageList.label}</span>
                </div>}
            </div>;
      })}
        {this.bench.length > 0 ? <div onClick={this.renderMoreFunc} className='base410-translate-funcs-button'>{_intl.t('更多')}</div> : null}
      </Fragment>;
  };
  renderMoreFunc = () => {
    this.setState({
  ...
  handlePatientId = patientId => {
    const {
      standardDese = false
    } = this.props.config; //标准脱敏规则

    if (standardDese) {
      return patientId.slice(0, 2) + '***' + patientId.slice(patientId.length - 4);
    }
  ...
  getCardNoByCardType = cardNo => {
    ...
    if (window.hospitalId == '961') {
      return cardNo.substring(0, 3) + '****' + cardNo.substring(cardNo.length - 3);
    }

    if (window.hospitalId == '1312') {
      const {
        formatPatientCardWhenLengthGE10 = false
      } = this.props.config;
      ...
      if (formatPatientCardWhenLengthGE10) {
        ...
        let {
          patientCard = ''
        } = info; // 2020.10.31 新增 如为 社保卡 展示原卡号
        ...
        if (patientCardQrCodeTextLength >= 10) {
          patientCard = String(patientCard).substring(0, 4) + '**' + String(patientCard).substring(patientCardQrCodeTextLength - 4);
        }
    ...
    if (['157', '1310'].includes(String(window.hospitalId)) && ['24'].includes(String(cardType))) {
      const {
        info = {}
      } = this.props.user;
      const {
    ...
    if (['157', '162', '163', '1310', '900'].includes(String(window.hospitalId))) {
      if (cardType == '1') {
        return cardNo.substring(0, 4) + '********' + cardNo.substring(cardNo.length - 4);
      }
    }
  ...
  renderTitle410 = () => {
    let {
      ...
      showPatientTypePrefix = _intl.t('病人类型:')
    } = this.props.config;
    let {
      info = {}
    } = this.props.user;
    ...
    if (['1040', '1045'].includes(String(window.hospitalId)) && useForcePayFor580 && !forcePayCompleted) {
      exitBtnStatus = false;
    }

    let cardNoText = this.getCardNoByCardType(info.cardNo || cardNo || '');
    let patientType = userInfo.patientType || '';
    let patientTypeName = userInfo.extendMap ? userInfo.extendMap.patientTypeName || '' : '';
    ...
    return <div className='index-info-base410-item'>
        {showPatientName && !showDeseName && <span className='index-info-base410-item-1'>
            {this.getPatientName(deseName || '')}
          </span>}
        {showPatientName && showDeseName && <span className='index-info-base410-item-1'>{finalName || ''}</span>}
        {showCardNo && !!cardNoText && <span className='index-info-base410-item-2'>{_intl.t('卡号：')}{cardNoText}</span>}
        {showUserId && !!userId && <span className='index-info-base410-item-2'>{_intl.t('门诊号：')}{userId || ''}
          </span>}
        {showPatientId && !!patientId && <span className='index-info-base410-item-2'>{_intl.t('门诊号：')}{patientIdText || ''}
          </span>}
        {showAccBalance && 'accBalance' in userInfo && <span className='index-info-base410-item-2'>{_intl.t('余额：')}{(Number(accBalance) / 100).toFixed(2) || ''}{_intl.t('元')}</span>}
        {showUserSex && <span className='index-info-base410-item-2'>{_intl.t('性别:')}{userInfo.sex}
          </span>}
        {showUserAge && <span className='index-info-base410-item-2'>{_intl.t('年龄:')}{userInfo.age || ''}
          </span>}
        {showPatientType && !!patientType && <span className='index-info-base410-item-2'>
  ...
  renderWaitToDo = () => {
    ...
    if (useTodoDrawer && user.isLogin && todoDrawerBtnOptBool) {
      return <Fragment>
          ...
          <Button className={`index-info-base410-waittoDo ${saveList.length == '0' ? 'unavailable' : ''}`} onTouchEnd={() => this.toggleDrawer()}>{_intl.t('待办')}</Button>
        </Fragment>;
    }
  ...
  renderMoreSlot = () => {
    ...
    return <div className={`new-index-active-functions-box total-length-eq-${secondFuncList.length}`}>
        {!!secondFuncList.length ? this.renderSecondFunc() : <IndexNotice410 {...noticeConfig410} />}
      </div>;
  };
  ...
  render() {
    ...
    let {
      useThemeVersion = false,
      // 是否使用580新主题，默认false
      useReadingPageAsIndex = false,
      // 是否使用 读卡动画作为新的首页， 即，首页轮询，并在查询到病人信息后 展示主页, 默认为false
      useTodoDrawer = true,
      showForceExitCardBtn = false,
      showForceExitCardBtnForV420 = false,
      // v4_2_0.html 是否展示 强制退卡按钮
    ...
    let functionList = toFixedColumns(indexPage['function'] || [], indexPage.functionColumns);
    let {
      infoIndex
    } = this.getLayout({
      showHeaderTime,
    ...
    let specialMainStyle = this.mainStyleList || []; //4.0签到屏  湖州中心用
    ...
    return <div>
        ...
        {showTips && <Tips showTips={showTips} click={click} handleCancel={this.handleCancel.bind(this)} handleComfirm={this.handleComfirm.bind(this)} />}
        {useReadingPageAsIndex && this.props.store.showReadgingPageAsIndex && <IndexReadingPage />}
        {(checkoutLoading || globalLoading || queryUserInfoLoading) && <Loading />}
        {useUserInfoScanner && !!userInfoScannerList && <UserInfoScanner />}
        {
        /* 通用部分 end */
      }
        {useType4_1_0 ? <Fragment>
            {<UserElectronicCardScanner />}
            {useForcePayFor580 && forcePay && this.ForcePayFor1040()}
            {!carReadResult.success ? <div className='new-indexpage-base410-title-contianer'>
                <div className='new-indexpage-base410-title'>{_intl.t('请选择您要办理的业务')}</div>
                {showForceExitCardBtnForV420 && <Button className='new-indexpage-base410-CardCheckout-in-title-container' onTouchEnd={() => this.handleForceExitCard()}>{_intl.t('强制退卡')}</Button>}
              </div> : infoIndex && this.renderTitle410()}
            <div className='new-indexpage-base410-card'>
              {!!fixedFuncList.length && fixedFuncList.map((item, key) => {
            let num = key - 6; // 卡纸条件：目前以义乌中心医院1002为准

            return <div key={key} style={specialMainStyle[key]} className={`
                ${item.hardwareStatus_receipt === false && item.hardwareCode_receipt === 1 ? 'PE' : ''} 
                ${item.hardwareStatus_receipt === true && item.hardwareCode_receipt === 8 ? 'PJ' : ''} 
                ${item.hardwareStatus_report === false && item.hardwareCode_report === 1 ? 'PE' : ''} 
                ${item.hardwareStatus_report === false && item.hardwareCode_receipt === 8 ? 'PJ' : ''}
                base410-func-parent-item 
                ${usePureColorForIndexPage ? 'base410-func-parent-item-pure-color' : ''}
                ${item.available === false ? 'function-unavailable' : ''} 
                ${useThemeVersion ? 'use-theme' : ''}
                ${!!item.location.background ? `base410-func-parent-item-${item.location.background}` : `base410-func-parent-item-${key >= 6 ? num : key}`}`}>
                      <div data-use-asr key={key} onClick={this.onFunctionClick.bind(this, item)} className={`special-func-item-base410 ${(specialStyle[key] || {}).cardWidthLevel <= 1 ? 'card-width-level-lte-1' : ''} ${!!item.location.background ? `base410-func-item-${item.location.background}` : `base410-func-item-${key >= 6 ? num : key}`}`} style={specialStyle[key]}>
                        <div className={`special-func-item-space-base410 ${useThemeVersion ? 'use-theme' : ''}`}>
                          <img className={`special-func-item-space-base410-icon ${useThemeVersion ? 'use-theme' : ''}`} src={useThemeVersion ? iconIndexMapUseTheme[item.iconIndex] : iconIndexMap[item.iconIndex]} alt='' />
                        </div>
                        <div>
                          <span>{item.label}</span>
                        </div>
                        {item.funcTips && <div className='special-func-item-space-base410-tips' style={item.funcTipsStyle || {}} dangerouslySetInnerHTML={{
                  __html: item.funcTips
                }}></div>}
                      </div>
                    </div>;
          })}
          ...
          /* 点击显示二级功能栏 */
            ...
            {showSecondList && <div className='base410-index-second-functions-onShadow'>
                <div className='base410-index-more-second-funcs'>
                  <div className='base410-index-more-second-func-title'>{_intl.t('更多功能')}</div>
                  {
              /* <div className="base410-index-more-second-func-coundown"></div> */
                ...
                return <div key={index} data-use-asr onClick={this.onFunctionClick.bind(this, item)} className='base410-index-more-second-func-item'>
                          {!!item.iconIndex || !!item.iconIndexImgUrl ? <div className='base410-second-func-item-3'>
                              <div>
                                <img className='base410-second-func-item-img' src={!!item.iconIndexImgUrl ? item.iconIndexImgUrl : useThemeVersion ? iconIndexSecondMap[item.iconIndex] : iconIndexSecondMap[item.iconIndex]} alt='' />
                              </div>
                              <div>
                                <span>{item.label}</span>
                              </div>
                            </div> : <div className='base410-second-func-item-4'>
                              <span>{item.label}</span>
                            </div>}
                        </div>;
              })}
                  </div>
                  <Button className='base410-second-func-item-cancel-button' onTouchEnd={this.cancelSecondList}>{_intl.t('返回')}</Button>
                </div>
              </div>}
            {(!todoDrawerJudge || this.state.todoDrawerJudge) && useTodoDrawer && !useThemeVersion && carReadResult.success && user.isLogin && todoDrawer && Object.keys(todoDrawer).length > 0 && <TodoDrawer visible={drawerVisible} {...todoDrawer} />}
            {showForceExitCardBtn && ['1040', '1045'].includes(String(window.hospitalId)) && <button className='base410-force-exit-card' onClick={this.handleForceExitCard}>{_intl.t('强制退卡')}</button>}
            {
          /* 更多功能 */
        }
            {this.state.moreFunctionsCard && <SubPageModal title={_intl.t('更多功能')} handleSubPageModalClose={() => this.setState({
          moreFunctionsCard: false
        })} hideConfirm={false}>
                <MoreFunctions onFunctionClick={this.onFunctionClick.bind(this)} />
              </SubPageModal>}
          </Fragment> : <Fragment>
            <IndexNotice {...noticeConfig} />
            <div className={indexPage.functionContainerClass}>
              {functionList.map((func, k) => func ? <Card key={k} {...func} onClick={this.onFunctionClick.bind(this, func)} // className={indexPage.functionClass}
          className={`${indexPage.functionClass} ${func.available === false ? 'unavailable' : ''}`} activeClass={indexPage.functionActiveClass} /> : <div key={k} className={indexPage.functionClass + '-fixed'} />)}
            </div>
            <div className='row-flex row-flex-center index-page-footer-btn'>
              {showForceExitCardBtn && <button className='blue-button index-page-footer-force-exit-btn' onClick={this.handleForceExitCard}>{_intl.t('强制退卡')}</button>}
              {this.renderExitCardBtnSlot()}
            </div>
            {useTodoDrawer && !useThemeVersion && carReadResult.success && user.isLogin && todoDrawer && Object.keys(todoDrawer).length > 0 && <TodoDrawer // visible={!drawerVisible ? (drawerVisible) : (drawerVisible || this.state.drawerFlag)}
        visible={drawerVisible} // forceVisible={this.state.drawerFlag}
